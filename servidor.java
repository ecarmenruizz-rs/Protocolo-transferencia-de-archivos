...
public class servidor {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        int tiempoProcessCliente = 0; //tiempo de espera al cliente (si es cliente lento se aumentará)
        ...
        try {
            ...
            while (true) {
                ...
                clienteSocket.setSoTimeout(4000);        // nº de ms que espera recibir ACKs antes de considerar cliente lento (TCP nos asegura que todo llega, por lo que por lo unico que se puede perder un paquete es porque lo desheche el cliente)
                ...
                if (IDComprobar) { // el cliente ha introducido la ID correcta
                    ...
                    do {
                        ...
                        for (int j = 0; j < numeroPalabrasSinACK; j++) {
                            ...
                            if (i * numeroPalabrasSinACK + j < texto_div.length) { //controlo que el indice no se salga de la longitud del vector texto
                                ...
                                Thread.sleep(tiempoProcessCliente); //Espera tiempoProcessCliente ms, mayor que 0 si se detecta cliente lento
                            }

                        }
                        //if ((i + 1) * numeroPalabrasSinACK >= texto_div.length) { //reinicio el texto para simular que es un flujo constante
                        //    i = 0;
                        //}
                        ...
                        try {
                            ...
                            if ("CTRL 1".equals(mensaje)) // se ha aceptado el procedimiento cliente lento
                                tiempoProcessCliente = ClienteLento(in, out);
                            
                            else                          // si no responde el cliente, se ha debido desconectar
                                conectado = false;
                        }

                        i++; //pasa al siguiente grupo de palabras

                    } while ("CTRL 4".equals(ACKrecibido) && conectado);
    ...
    public static boolean ContinuarTransmision() {
        return true;
    }

    public static int ClienteLento(DataInputStream in, DataOutputStream out) throws IOException {
        int tiempoEspera;
        long acumulador = 0;
        String texto = "Texto de prueba para cliente lento, por favor espere a que se reanude";
        String[] texto_div = texto.split(" ");

        for (int j = 0; j < texto_div.length; j++) {

            out.writeUTF("Msg " + texto_div[j]);
            long tiempoInicial = System.nanoTime();
            long tiempoFinal;
            boolean equals = texto_div[j].equals(in.readUTF());
            tiempoFinal = System.nanoTime();
                
            
            acumulador += tiempoFinal - tiempoInicial;
        }
        tiempoEspera = (int) (acumulador / texto_div.length);
        return tiempoEspera;
    }
}
